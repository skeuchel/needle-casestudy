...
Proof.
  induction 1; try (econstructor; eauto using K_TVar; fail); eapply Q_Trans;
    [ eauto using Q_App, Q_Abs | eauto using Q_Beta, tyeq_kinding_right ].
Qed.
...
Proof.
  ...
  - constructor; auto.
    specialize (IHwt (wf_evar wfΓ jm5)).
    eapply strengthening_evar_kinding; eauto.
    exact (subst_evar_here _ _ t).
  - specialize (IHwt1 wfΓ).
    dependent destruction IHwt1; auto.
  - constructor; eauto with infra.
    specialize (IHwt (wf_etvar wfΓ)).
    eauto using strengthening_evar_kinding.
  - specialize (IHwt wfΓ).
...
Proof.
  intros P Q; revert S P; induction Q; eauto using QRS_Cons.
Qed.
...
Proof.
  depind r; simpl; econstructor; eauto using Kinding, TRed with infra.
Qed.
...
Proof with simpl; eauto using QR_Refl, TRed.
  ...
  - eauto using subst_tred_tred with wf infra.
Qed.
...
Proof.
  ...
  - destruct (IHr V SV) as (W & TV & VW).
    destruct (tred_single_step_diamond TV jm36) as (Z & UZ & VZ).
...
Proof.
  ...
  - destruct (IHST _ SV) as (W & TW & VW).
    destruct (tred_strip TW _ jm36) as (Z & WZ & UZ).
    eauto using QRS_Cons.
Qed.
...
Proof.
  ...
  - dependent destruction jm34. eauto 10 using TRedStar.
  - destruct (IHTRedStar) as [T1 [T2 [Teq [S1T1 S2T2]]]]; subst.
    dependent destruction jm36; exists T0, T3; eauto 20 using TRedStar.
Qed.
...
Proof.
  depind H; simpl; intros.
  - dependent destruction jm34.
    exists S2; repeat split; auto; constructor; auto.
  - destruct (IHTRedStar) as [T1 [Teq S2T2]]; subst.
    inversion jm36; exists T2; repeat split; eauto 20 using QRS_Cons.
Qed.
...
Proof.
  ...
  eapply Q_Refl; eauto using wf_empty, typing_kinding with infra.
Qed.
...
Proof.
  ...
  - destruct (abs_inversion wΓ wt1) as [eq11 wt12].
    eauto using T_Eq, subst_evar_Typing with infra.
  - destruct (tabs_inversion wΓ wt) as [e11 wt12]; subst;
